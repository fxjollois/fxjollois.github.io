...
[sourceTP5]: https://github.com/fxjollois/fxjollois.github.io/blob/master/prog-stat-r-tp5.Rmd "Fichier source du TP5, disponible sur github"
...
1. Créer un nouveau document `R markdown`, intitulé `"TP5"`, avec votre nom comme auteur
1. Sauvegarder ce document sous le nom `"TP5.Rmd"`
...
1. Créer 4 titres de niveau 2, avec le contenu suivant
    ...
    3. **Consommation**
        - Histogramme de la consommation (variable `mpg`)
    ...
    2. **Transmission** :
        ...
        - afficher le diagramme circulaire de la répartition des voitures en fonction de leur transmission avec les paramètres suivants :
            - la couleur `"orange"` pour `"automatic"` et `"blue"` pour `"manual"`
            - un titre de figure indiquant ce qui est représenté
            - centrage dans le document (cf `fig.align`)
            - largeur et hauteur de `2.5`(cf `fig.width` et `fig.height`), si possible en réduisant les marges dans la figure (commande `par` et paramètre `mar` dans celle-ci)
    4. **Consommation et transmission**
        - afficher les boîtes à moustaches de la consommation en fonction de la transmission, avec les conrtaintes suivantes 
            - couleurs indiquées ci-dessus
            - titre cohérent avec la représentation
            - figure centrée et avec des marges réduites
            - taille modérée
