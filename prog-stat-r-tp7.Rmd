...
```{r}
d = aggregate(cbind(ca2003, ca2004, evolution) / 1000 ~ departement, ca_evol, sum)
names(d) = c("Département", "2003", "2004", "Evolution")
knitr::kable(d, digits = 0)
```

```{r}
d = aggregate(cbind(ca2003, ca2004, evolution) / 1000 ~ provenance, ca_evol, sum)
names(d) = c("Provenance", "2003", "2004", "Evolution")
knitr::kable(d, digits = 0)
```
...
Pour publier un tableau de bord ainsi créé, il faut juste placer le document `.Rmd`, ainsi que les données si besoin, sur un serveur `shiny` dédié. Vous pouvez voir un exemple avec [ce lien](http://193.51.82.116:3838/demo/prog-stat-r-tp7/).
