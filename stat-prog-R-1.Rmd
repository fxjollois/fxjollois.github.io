...
Data description can be found [here](http://archive.ics.uci.edu/ml/datasets/Pen-Based+Recognition+of+Handwritten+Digits): each line represent a draw of a number between 0 and 9, with 8 points ($(x,y)$ coordinates for each point) and the number drawn.

```{r, echo = FALSE}
pen.tra = read.table("donnees/pendigits.tra", sep = ",")
pen.tes = read.table("donnees/pendigits.tes", sep = ",")
pen = rbind(pen.tra, pen.tes)
names(pen) = c(paste0(c("X", "Y"), rep(1:8, each = 2)), "number")
```
...
1. Describe (quickly) all attributes
    - one by one first
    - coordinates with respect to `number`
1. Represent the first `"0"` drawn as following
```{r, echo = FALSE}
first.0 = subset(pen, number == 0)[1,]
...
par(mfrow = c(2, 5))
for (i in 0:9) {
    s = subset(pen, number == i)[1,]
    drawn(s[-16], s[17])
}
```
1. Compute the mean drawn for each number and represent it, with each drawn in (grey) light gray
```{r, echo = FALSE}
par(mfrow = c(2, 5))
for (i in 0:9) {
    s = subset(pen, number == i)
...
plot(res$ind$coord, pch = 19, cex = .5, col = rainbow(10)[pen$number + 1],
...
par(mfrow = c(2, 5))
for (i in 0:9) {
    s = res$ind$coord[pen$number == i,]
    par(mar = c(2, 2, 2, 0) + .1)
    plot(s, pch = 19, cex = .5, col = rainbow(10)[i + 1],
         xlim = c(-5, 4), ylim = c(-5, 5), main = i)
}
```
1. We conclude here that we surely need to clusters data separatly for each number, to detect if there are different ways to write a number (and how).
