...
* **Cinq intervenants** :
    + Servane Gey (9 Mars) et Charles Bouveyron ( Mars) : introduction aux méthodologies
...
* **Deux intervenants** :
    + Servane Gey (9 Mars) Servane.Gey@parisdescartes.fr
...
Recensement aux Etats-Unis en 2012
...
$\Sigma$ matrice des variances/covariances (resp. des corrélations) : $\Sigma_{k,j}=Cov(X^{(k)},X^{(j)})$ (resp. $\Sigma_{k,j}=Cor(X^{(k)},X^{(j)})$)
...
Données brutes : Tableau disjonctif complet = recodage des variables qualitatives en autant de variables binaires qu'il y a de modalités.

Données groupées bivariées : tableau de contingence = distribution jointe des deux variables qualitatives $X$ et $Y$.

```{r}
table(x$SEXE,x$REV_FOYER)
```

Données groupées bivariées : profils lignes = distributions de la variable colonne $Y$ conditionnellement aux modalités de la variable ligne $X$.

```{r}
100 * prop.table(table(x$SEXE,x$REV_FOYER), margin = 1)
```

Données groupées bivariées : profils colonnes = distributions de la variable ligne $X$ conditionnellement aux modalités de la variable colonne $Y$.
...
Données groupées multivariées : Table de Burt. Utile lorsqu'il s'agit de stocker une base de données (enquête, etc...) composée d'un grand nombre d'individus.
...
![Fruits](analyse-donnees/fruits.jpg)
...
- Construction basée sur la **diagonalisation** de la matrice des variances/covariances (ACP non-normée), ou des corrélations (ACP normée).
...
## Lecture et interprétation des valeurs propres (2) 
...
Pour 1724 femmes interrogées, tableau croisant l'image de la famille idéale et l'activité convenant le mieux à une femme dont les enfants vont à l'école.
...
Enquête effectuée sur 300 consommateurs de thé concernant leurs habitudes de consommation.
...
* Etude des données de fertilité chez 100 hommes volontaires 
* Charger les données dans R
...
* Matrice des distances kilométriques entre 47 villes de France. 
* Charger les données dans R
