...
CIS_surv = subset(CIS, Surveillance == "Oui")
...
```{r}
# En R
res = aggregate(Code ~ CodeCIS + Denomination, 
                merge(CIS, subset(CIS_COMPO, select = c(CodeCIS, Code)), by = "CodeCIS"), 
                length)
knitr::kable(res[head(order(res$Code, decreasing = TRUE), 10),], row.names = FALSE)
```
...
library(sqldf)
res = sqldf("
SELECT CIS.CodeCIS, CIS.Denomination, COUNT(*) AS NbComposants
    FROM CIS INNER JOIN CIS_COMPO ON CIS.CodeCIS = CIS_COMPO.CodeCIS
    GROUP BY CIS.CodeCIS
    ORDER BY COUNT(*) DESC
    LIMIT 10;
      ")
res
...
res = merge(
    merge(
        setNames(aggregate(CodeCIS ~ Libelle, temp, length), c("Groupe", "Nombre")),
        setNames(aggregate(Prix ~ Libelle, temp, mean), c("Groupe", "Prix moyen")),
        all = TRUE
    ),
    setNames(aggregate(Taux ~ Libelle, temp, mean), c("Groupe", "Taux moyen")),
    all = TRUE
)    
...
```{r}
voies = unique(unlist(strsplit(CIS$VoiesAdm, ";")))
temp = data.frame(CodeCIS = CIS$CodeCIS,
                  matrix(FALSE, nrow(CIS), length(voies)))
names(temp)[-1] = voies
for (i in 1:nrow(CIS)) {
    temp[i,-1] = (voies %in% unlist(strsplit(CIS$VoiesAdm[i], ";")))
}
```
