...
```{r qplot-quali-quali}
qplot(factor(am), facets = 1 ~ cyl, data = mtcars, geom = "bar")
qplot(factor(am), facets = 1 ~ cyl, fill = factor(am), data = mtcars, geom = "bar")
...
ggplot(mtcars, aes(factor(am))) + geom_bar() + facet_grid(1 ~ cyl)
...
ggplot(melt(iris, id.vars = "Species"), aes(Species, value)) + 
    geom_boxplot() + facet_grid(1 ~ variable)
```
...
ggplot(tips, aes(sex, smoker, z = total_bill)) + 
    stat_summary_2d() 
-->
2. Toujours dans `tips`, représenter `total_bill` et `tip`, en fonction de `sex` de 2 façons
<!--
...
-->
4. Comment répondre aux questions suivantes :
...
5. En utiliser les données `crime` du package `ggmap`, comment représenter le nombre et les types de crime par rapport à leur localisation ?
...
```{r airbnb-geo}
airbnb.neigh = geojson_read("airbnb-paris-2015-09-02-neighbourhoods.geojson", what = "sp")
