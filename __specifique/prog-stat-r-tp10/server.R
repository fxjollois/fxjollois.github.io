...
shinyServer(function(input, output) {
  ...
  output$nuage1 <- renderPlot({
    plot(mtcars$mpg ~ mtcars[,input$choix], 
         main = "Consommation",
  ...
  output$qlql.bar <- renderPlot({
    tab = data.frame(
      x = factor(mtcars[,input$qlql.choix1]),
  ...
  output$qlql.tab <- renderTable({
    tab = table(
      mtcars[,input$qlql.choix1],
      mtcars[,input$qlql.choix2]
    )
    dimnames(tab)[[1]] = paste(
      input$qlql.choix1, dimnames(tab)[[1]], sep = "="
    )
    dimnames(tab)[[2]] = paste(
      input$qlql.choix2, dimnames(tab)[[2]], sep = "="
    )
