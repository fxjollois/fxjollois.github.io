...
```{r, echo=FALSE}
setwd("/Users/fxjollois/Sites/fxjollois.github.io")
```
...
```{r}
iris_bis = read.table("donnees/Iris.txt", header = T, sep = ";")
head(iris_bis)
```
...
```{r}
write.table(iris_bis, file = "donnees/Iris_bis.txt", sep = "%", 
...
- les noms des variables sont dans le fichier [`adult.names`](donnees/adult.names) présenté en dessous

<object data="donnees/adult.data" type="text/javascript" style="width: 100%">
    impossible à afficher
</object>
<object data="donnees/adult.names" type="text/javascript" style="width: 100%">
    impossible à afficher
</object>
